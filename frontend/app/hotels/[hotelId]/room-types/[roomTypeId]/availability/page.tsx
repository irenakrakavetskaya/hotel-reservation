...
import { ApiError, type Availability } from '../../../../../../lib/api';
import { getAccessToken, getAvailability, getHotel, getRoomType } from '../../../../../../lib/server-api';
...
export default async function AvailabilityPage({ params, searchParams }: AvailabilityPageProps) {
  ...
  try {
    ...
    if (initialStartDate && initialEndDate && initialRoomCount) {
      availability = await getAvailability(hotelId, roomTypeId, initialStartDate, initialEndDate, initialRoomCount);
    }
  } catch (error) {
  ...
  return (
    <main className="page-shell page-shell--compact">
      ...
      <BookingForm hotel={hotel} roomType={roomType} token={token} initialStartDate={initialStartDate} initialEndDate={initialEndDate} initialRoomCount={initialRoomCount} initialAvailability={availability} />
    </main>
  );
}
